...
"""
╔══════════════════════════════════════════════════════════════════════════════╗
║  SymbolicDNA_Forge_Chimera2a.py                                             ║
║  ────────────────────────────────────────────────────────────────────────    ║
...
class FengShuiDisplay:
    """Gestionnaire d'affichage harmonieux pour les messages du programme."""
    
    # Couleurs ANSI pour un affichage apaisant
    COLORS = {
        'reset': '\033[0m',
        'gold': '\033[38;5;214m',
        'jade': '\033[38;5;41m',
    ...
    def _colorize(cls, text: str, color: str, *extra) -> str:
        """Applique une ou plusieurs couleurs ANSI au texte."""
    ...
    def _wrap_paragraph(cls, text: str, width: int = 80) -> str:
        """Enveloppe le texte pour un affichage propre."""
        words = text.split()
        lines = []
        current = []
        current_len = 0
    ...
    def mantra(cls, text: str, width: int = 70):
        """Affiche un mantra encadré avec élégance."""
        wrapped = cls._wrap_paragraph(text, width - 8)
        lines = wrapped.split('\n')
        deco = cls._colorize('◈', 'gold')
...
# ═══════════════════════════════════════════════════════════════════════════════
# DATA LOADER — Chargement des lexiques depuis fichiers JSON externes avec fallback
# ═══════════════════════════════════════════════════════════════════════════════

def load_oniric_lexicon() -> Dict:
    """
    Charge le lexique onirique depuis un fichier JSON externe.
    Si le fichier est absent ou invalide, utilise les données intégrées en fallback.
    """
    ...
    # Chemins possibles pour les fichiers JSON
    json_paths = [
        "data/oniric_lexicon.json",
        "./oniric_lexicon.json",
        os.path.join(os.path.dirname(__file__), "data", "oniric_lexicon.json"),
        os.path.join(os.path.dirname(__file__), "oniric_lexicon.json"),
    ]
    ...
    for path in json_paths:
        if os.path.exists(path):
            try:
                ...
                required_keys = ["Adjectif", "Nom", "Action", "Symbole", "oniric_tags"]
                if all(k in loaded for k in required_keys):
                    FengShuiDisplay.info(f"📖 Lexique chargé depuis {path}", "🪷")
                    return loaded
                else:
                    FengShuiDisplay.warning(f"Le fichier {path} est invalide. Utilisation du fallback.")
            except json.JSONDecodeError:
                FengShuiDisplay.warning(f"Le fichier {path} n'est pas un JSON valide. Utilisation du fallback.")
            except Exception as e:
                FengShuiDisplay.warning(f"Impossible de charger {path}: {e}. Utilisation du fallback.")
    
    FengShuiDisplay.info("📖 Utilisation du lexique intégré (fallback)", "🪷")
    return fallback


def load_oniric_tag_meanings() -> Dict[str, str]:
    """
    Charge les significations des tags oniriques depuis un fichier JSON externe.
    """
    fallback = {
    ...
    json_paths = [
        "data/oniric_tag_meanings.json",
        "./oniric_tag_meanings.json",
        os.path.join(os.path.dirname(__file__), "data", "oniric_tag_meanings.json"),
        os.path.join(os.path.dirname(__file__), "oniric_tag_meanings.json"),
    ]
        ...
        if os.path.exists(path):
            try:
                ...
                if isinstance(loaded, dict):
                    FengShuiDisplay.info(f"📖 Tags oniriques chargés depuis {path}", "🪷")
                    return loaded
                else:
                    FengShuiDisplay.warning(f"Le fichier {path} est invalide. Utilisation du fallback.")
            except Exception as e:
...
def load_theme_templates() -> Dict[str, List[str]]:
    """
    Charge les templates de thèmes depuis un fichier JSON externe.
    """
    fallback = {
        'protection': [
    ...
    json_paths = [
        ...
        os.path.join(os.path.dirname(__file__), "data", "theme_templates.json"),
        os.path.join(os.path.dirname(__file__), "theme_templates.json"),
    ]
    ...
    for path in json_paths:
        if os.path.exists(path):
            try:
                with open(path, 'r', encoding='utf-8') as f:
                    loaded = json.load(f)
                if isinstance(loaded, dict) and all(k in loaded for k in fallback.keys()):
                    FengShuiDisplay.info(f"📖 Templates de thèmes chargés depuis {path}", "🪷")
                    return loaded
                else:
                    FengShuiDisplay.warning(f"Le fichier {path} est invalide. Utilisation du fallback.")
            except Exception as e:
...
def load_theme_symbol_pools() -> Dict[str, List[str]]:
    """
    Charge les pools de symboles par thème depuis un fichier JSON externe.
    """
    fallback = {
    ...
    json_paths = [
        "data/theme_symbol_pools.json",
        "./theme_symbol_pools.json",
        os.path.join(os.path.dirname(__file__), "data", "theme_symbol_pools.json"),
        os.path.join(os.path.dirname(__file__), "theme_symbol_pools.json"),
    ]
        ...
        if os.path.exists(path):
            try:
                with open(path, 'r', encoding='utf-8') as f:
                    loaded = json.load(f)
                if isinstance(loaded, dict) and all(k in loaded for k in fallback.keys()):
                    FengShuiDisplay.info(f"📖 Pools de symboles chargés depuis {path}", "🪷")
                    return loaded
                else:
                    FengShuiDisplay.warning(f"Le fichier {path} est invalide. Utilisation du fallback.")
            except Exception as e:
...
def load_theme_palettes() -> Dict[str, List[str]]:
    """
    Charge les palettes chromatiques par thème depuis un fichier JSON externe.
    """
    fallback = {
    ...
    json_paths = [
        "data/theme_palettes.json",
        "./theme_palettes.json",
        os.path.join(os.path.dirname(__file__), "data", "theme_palettes.json"),
        os.path.join(os.path.dirname(__file__), "theme_palettes.json"),
    ]
        ...
        if os.path.exists(path):
            try:
                with open(path, 'r', encoding='utf-8') as f:
                    loaded = json.load(f)
                if isinstance(loaded, dict) and all(k in loaded for k in fallback.keys()):
                    FengShuiDisplay.info(f"📖 Palettes chargées depuis {path}", "🪷")
                    return loaded
                else:
                    FengShuiDisplay.warning(f"Le fichier {path} est invalide. Utilisation du fallback.")
            except Exception as e:
...
ONIRIC_TAG_MEANINGS = load_oniric_tag_meanings()
THEME_TEMPLATES = load_theme_templates()
THEME_SYMBOL_POOLS = load_theme_symbol_pools()
THEME_PALETTES = load_theme_palettes()
...
# Le lexique est maintenant chargé dynamiquement via les fonctions ci-dessus.
# Les références à LEXICON sont conservées.
...
class SymbolicEvolutionEngine:
    ...
    def initialize_population(self):
        FengShuiDisplay.info("🌱 Germination de la population initiale...", "🌿")
        self.population = [self._spawn_organism(theme=self.theme) for _ in range(self.population_size)]
        FengShuiDisplay.success(f"Population de {self.population_size} organismes générée.", "🌱")
    ...
    def evolve(self, generations: int = 5, chaos_probability: float = 0.1, verbose: bool = True):
        ...
        FengShuiDisplay.section(f"Évolution sur {generations} générations", "🔄")
        ...
        for gen in range(generations):
            ...
            if verbose:
                FengShuiDisplay.info(f"Génération {self.generation_count:03d} | {best_genome.breed} | fitness: {best_fitness:.3f}", "○")
                # Affichage du mantra en mode poème
                mantra_lines = best_artefacts[0].mantra_text.split()
                if len(mantra_lines) > 15:
                    mantra_lines = mantra_lines[:15] + ["..."]
                FengShuiDisplay.poem([" ".join(mantra_lines)], title="📜 Mantra")
    ...
    def export_organism(self, genome: SymbolicOrganismGenome, artefacts: List[SymbolicArtefact],
        ...
        FengShuiDisplay.section("Exportation des artefacts", "📦")
        ...
        FengShuiDisplay.info(f"📊 {json_path}", "○")
...
def build_dna_from_args(args, theme: str = "rituel") -> SymbolicDNA:
    ...
    if hasattr(args, 'emotion') and args.emotion:
        try:
            ...
            FengShuiDisplay.error("Erreur de décodage du vecteur d'émotions. Utilisation des valeurs par défaut.")
...
"""
    ...
    FengShuiDisplay.header(
        "SymbolicDNA_Forge_Chimera v2.0",
        "La Forge de l'ADN Symbolique — Évolution narrative et visuelle"
    )

    FengShuiDisplay.section("Paramètres d'invocation", "⚙️")
    
    # Affichage des paramètres sous forme d'arbre
    params = [
        ("Thème", args.theme, "gold"),
        ("Population", args.population, "jade"),
        ("Générations", args.generations, "sapphire"),
        ("Chaos", f"{args.chaos:.2f}", "rose"),
        ("Graines", args.seed or "aléatoire", "silver"),
    ]
    if args.diffusion_prompt:
        params.append(("Prompt", f"oui (cible: {args.diffusion_target})", "amethyst"))
    FengShuiDisplay.tree(params, "🌊 Configuration")
    ...
    if custom_params:
        FengShuiDisplay.section("ADN symbolique personnalisé", "🔧")
        ...
        FengShuiDisplay.info(f"🧬 Glyphe: {custom_dna.glyph_symbol} | Couleur: {custom_dna.color}", "○")
        FengShuiDisplay.info(f"📐 Complexité: {custom_dna.complexity:.2f} | Symétrie: {custom_dna.symmetry}", "○")
        
        # Évaluation
        FengShuiDisplay.info("Évaluation de l'ADN personnalisé...", "○")
        ...
        FengShuiDisplay.success(f"Fitness: {best_fitness:.3f}")
        FengShuiDisplay.mantra(best_artefacts[0].mantra_text)
    else:
        FengShuiDisplay.section("Évolution générative", "🧬")
    ...
    if not args.no_visual:
        ...
        FengShuiDisplay.success(f"Tous les artefacts exportés dans: {args.out}")
    ...
    if args.diffusion_prompt:
        ...
        FengShuiDisplay.separator("═", 60)
        FengShuiDisplay.info("PROMPT GÉNÉRÉ", "📜")
        FengShuiDisplay.separator("─", 60)
        print()
    ...
    if not custom_params and not args.no_visual:
        ...
        FengShuiDisplay.info(f"🖼️  Planche de phylogénie: {board_path}", "○")

    # ============================================================
    # CONCLUSION FENG-SHUI
    # ============================================================
    FengShuiDisplay.separator("═", 60)
    FengShuiDisplay.section("Forge achevée", "✨")
    
    summary_items = [
        ("Fitness", f"{best_fitness:.3f}/1.0", "jade"),
        ("Symbole", best_genome.dominant_strand().glyph_symbol, "gold"),
        ("Couleur", best_genome.dominant_strand().color, "rose"),
        ("Émotion", best_genome.dominant_strand().dominant_emotion(), "amethyst"),
    ]
    FengShuiDisplay.tree(summary_items, "📋 Résumé")
    
    if args.diffusion_prompt:
        FengShuiDisplay.info(f"📝 Prompt disponible: {prompt_path}", "📜")
    
    print()
    FengShuiDisplay.poem([
        "Le code s'efface dans le silence,",
        "Le symbole danse dans la lumière,",
        "Le mantra résonne dans l'éther,",
        "La forge se repose en paix."
    ], title="🌸 Épilogue")
    print()
