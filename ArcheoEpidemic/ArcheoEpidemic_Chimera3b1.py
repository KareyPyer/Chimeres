...
"""
╔═══════════════════════════════════════════════════════════════════════════════╗
║   🧬🌌 ARCHEOEPIDEMIC CHIMERA — v2.1 "DATA_ORACLE"                          ║
║   Ajouts v2.1 :                                                             ║
║     • Export CSV multi-tables pour KNIME                                    ║
║     • Métriques temporelles enrichies                                       ║
║     • Traçabilité complète des interactions                                ║
║     • Données longitudinales par agent                                     ║
║     • Historique des mutations et dérives sémantiques                      ║
║     • Conservation de l'affichage RetroWave v2.0                          ║
║     • Affichage stabilisé (plus de lignes fugaces)                        ║
╚═══════════════════════════════════════════════════════════════════════════════╝
"""
import random
import math
...
# ═══════════════════════════════════════════════════════════════════════════════
# [NOUVEAU] AFFICHAGE RETRO WAVE — ASCII ART + ANIMATION
# ═══════════════════════════════════════════════════════════════════════════════
...
class RetroWaveDisplay:
    ...
    def __init__(self, width: int = 80):
        ...
        self._last_output_height = 0  # Pour suivre la hauteur de l'affichage
    ...
    ▐░░░░░░░░░░░▌▐░░░░░░░░░░░▌▐░░░░░░░░░░░▌▐░░░░░░░░░░░▌▐░░░░░░░░░░░▌
     ▀▀▀▀▀▀▀▀▀▀▀  ▀▀▀▀▀▀▀▀▀▀▀  ▀▀▀▀▀▀▀▀▀▀▀  ▀▀▀▀▀▀▀▀▀▀▀  ▀▀▀▀▀▀▀▀▀▀▀ 
        ...
        banner += self.c("   ▸ Export CSV multi-tables pour KNIME\n", "dim")
        banner += self.c("\n", "reset")
    ...
    def status_bar(self, sim) -> str:
        ...
        line1 = f"  {self.c('⏱', 'cyan')} t={t:>3}  {self.c('🧬', 'magenta')} souches={strains:>2}  {self.c('📜', 'yellow')} reliques={relics:>2}  {self.c('📖', 'green')} mythes={myths:>2}  {self.c('📈', 'red')} Rt={rt:>5.2f}  {self.c('👤', 'blue')} pop={total:>3}"
    ...
    def animated_step(self, sim, step: int, total_steps: int) -> str:
        ...
        lines.append(f"  {self.c('⏱', 'bright_yellow')} Écoulé : {elapsed:.1f}s  |  {self.c('⚡', 'bright_cyan')} Pas : {step}  |  {self.c('🧬', 'bright_magenta')} Souches : {len(sim.meme_strains)}")
    ...
    def render_report(self, sim, report: str) -> str:
        ...
        for line in report.split("\n"):
            ...
            elif "Reliques" in line or "Mythes" in line:
                lines.append(self.c(line, "bright_green", "bold"))
            elif "Chronique" in line or "Dérive" in line:
    ...
    def render_full(self, sim, step: int, total_steps: int, report: str = None) -> str:
        """Rendu complet avec rafraîchissement stabilisé — plus de lignes fugaces !"""
        
        # Effacement complet au premier pas
        if self._first_render:
            self.clear_screen()
            print(self.banner())
            self._first_render = False
            # Petit délai pour laisser la bannière s'afficher
            time.sleep(0.3)
        
        # Calcul du nombre de lignes à remonter
        # On remonte de 50 lignes pour être sûr de couvrir tout l'affichage
        print("\033[50A", end="")   # Monte de 50 lignes
        print("\033[J", end="")     # Efface jusqu'en bas
        
        # Génération de l'affichage
        output = self.animated_step(sim, step, total_steps)
        print(output, end="")
        
        # Si c'est la fin, afficher le rapport
...
# ═══════════════════════════════════════════════════════════════════════════════
# [FUSION] CulturalEpidemicSimulation — v2.1 avec export CSV
# ═══════════════════════════════════════════════════════════════════════════════
class CulturalEpidemicSimulation:
    def __init__(self, params: dict, genome_pool: Optional[List[CulturalGenome]] = None):
        ...
        # [NOUVEAU v2.1] Historique complet pour CSV
    ...
    # -------------------------------------------------------------------
    # [NOUVEAU v2.1] CAPTURE D'ÉTAT POUR CSV
    # -------------------------------------------------------------------
...
class CSVExporter:
    ...
    def export_all(sim: CulturalEpidemicSimulation, output_dir: str):
        ...
        # Génération d'un README pour KNIME
    ...
    def _export_semantic_drift(path: Path, sim: CulturalEpidemicSimulation):
        ...
        if not rows:
            # Ajout d'une ligne vide pour éviter l'erreur
...
def draw_cultural_network(sim: CulturalEpidemicSimulation):
    ...
    ax.set_title("Réseau narratif — ArcheoEpidemic Chimera v2.1", color="#cccccc", fontsize=11)
    return fig
...
def mythological_report(sim: CulturalEpidemicSimulation) -> str:
    lines = []
    lines.append("═" * 70)
    lines.append("  📖 RAPPORT MYTHOLOGIQUE — ArcheoEpidemic Chimera v2.1 DATA_ORACLE")
    lines.append("═" * 70)
    status_counts = Counter(a.cultural_status for a in sim.agents)
    total = len(sim.agents)
...
# ═══════════════════════════════════════════════════════════════════════════════
# [NOUVEAU v2.1] FONCTION PRINCIPALE AVEC EXPORT CSV
# ═══════════════════════════════════════════════════════════════════════════════
...
def run_cultural_epidemic_simulation(params: dict, genome_pool: Optional[List[CulturalGenome]] = None,
                                     ...
                                     export_csv: Optional[str] = None) -> CulturalEpidemicSimulation:
    """Version avec affichage rétro-wave et export CSV."""
    ...
    for i, snapshot in enumerate(sim.run(steps)):
        if retro_display and display:
            ...
            time.sleep(0.08)  # Petit délai pour l'effet d'animation
        elif verbose:
            m = snapshot["metrics"]
...
# ═══════════════════════════════════════════════════════════════════════════════
# [NOUVEAU v2.1] CLI EXHAUSTIVE AVEC EXPORT CSV
# ═══════════════════════════════════════════════════════════════════════════════

def build_arg_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="archeoepidemic_chimera",
        description="🧬🌌 ARCHEOEPIDEMIC CHIMERA v2.1 DATA_ORACLE — Simulateur d'épidémies narratives avec export CSV",
    ...
    exp.add_argument("--export-csv", type=str, default=None,
                     help="Répertoire d'export des CSV multi-tables pour KNIME")
    exp.add_argument("--export-json", type=str, default=None,
                     help="Répertoire d'export des données JSON (compatible v2.0)")
...
def main():
    ...
    logger.info("🌌🧬 Démarrage d'ArcheoEpidemic Chimera v2.1 — Normandie Fractale, 2075")
    logger.info("Fusion : Glyphosophia (mantras/glyphes) × Corrupted Blood (épidémie agent-based)")
    logger.info("Ajouts v2.1 : Export CSV multi-tables pour KNIME, données longitudinales")
    ...
    # Export JSON (compatibilité v2.0)
    if args.export_json:
        try:
            # Tentative d'import du module v2.0
